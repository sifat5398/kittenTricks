...
> Note: For fresh clones and CI builds, Babel and Metro now default to `node_modules/@ui-kitten` and `node_modules/@eva-design` if `env/index.js` is missing. If you need custom package paths, set `UI_KITTEN_PACKAGES_PATH` and `EVA_PACKAGES_PATH` in your environment or run `npm run env -- prod` locally.
